...
const tryLoadPinnedKmlLayerWhenReady = () => {
        ...
        reader.onload = async () => {
            ...
            try {
                ...
                if (parsedFeatures.length === 0) {
                    window.showMessage(
                        'KML 載入',
                        'KML 檔案中沒有找到任何可顯示的地理要素 (點、線、多邊形)。請確認 KML 檔案內容包含 <Placemark> 及其有效的地理要素。'
                    );
                    return;
                }
                ...
                if (!existingKmlQuery.empty) {
                    ...
                    // 清除舊的 features 子集合（舊版 v1.9 的遺留）
                    const oldFeaturesSnapshot = await kmlLayerDocRef.collection('features').get();
                    const deleteBatch = db.batch();
                    oldFeaturesSnapshot.forEach(doc => deleteBatch.delete(doc.ref));
                ...
                console.log(`已寫入整包 geojson（${parsedFeatures.length} 個 features）到文件 ${kmlLayerDocRef.id}`);
    ...
    deleteSelectedKmlBtn.addEventListener('click', async () => {
        ...
        const confirmDelete = await window.showConfirmationModal(
            '確認刪除 KML',
            '確定要刪除此 KML 圖層嗎？此操作不可逆！'
        );
    
        if (!confirmDelete) return;
    
        try {
            const kmlLayerDocRef = db
                .collection('artifacts')
                .doc(appId)
                .collection('public')
                .doc('data')
