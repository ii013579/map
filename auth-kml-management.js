...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 標記 pinned 是否已載入過
    window.alreadyLoadedPinned = false;
    
    /**
     * 嘗試載入 pinned 圖層
     */
    window.tryLoadPinnedKmlLayerWhenReady = function() {
        if (window.alreadyLoadedPinned) {
            console.log("⚠️ 已載入過 pinned，略過重複呼叫");
            return;
        }
    
        const pinnedId = localStorage.getItem("pinnedKmlId");
        if (pinnedId) {
            console.log(`📌 載入 pinned 圖層 ${pinnedId}`);
            window.alreadyLoadedPinned = true; // 🔒 避免重複
    ...
    window.updateKmlLayerSelects = async function() {
        ...
        if (!listSnapshot.empty) {
            ...
            listSnapshot.forEach(doc => {
                layerList.push({ id: doc.id, name: doc.data().name || doc.id });
            });
        } else {
            // 🔄 回退到舊結構 (kmlLayers)
            const layersRef = db.collection('artifacts').doc(appId)
                .collection('public').doc('data')
                .collection('kmlLayers');
            const layersSnapshot = await layersRef.get();
        ...
        layerList.forEach(layer => {
            ...
            kmlSelect.appendChild(option);
        });
            
            const snapshot = await kmlRef.get();
    
            kmlSelect.innerHTML = ""; // 清空選單
            ...
            console.log("✅ KML 下拉選單已更新");
    
            // ⚠️ 這裡不再每次都自動呼叫 pinned
            if (!window.alreadyLoadedPinned) {
                window.tryLoadPinnedKmlLayerWhenReady();
            }
    
        } catch (error) {
            console.error("更新 KML 下拉選單時出錯:", error);
        }
    };
        ...
        reader.onload = async () => {
            ...
            try {
                ...
                if (!existingKmlQuery.empty) {
                    ...
                    // ✅ 這裡插入「同步更新 kmlList」區塊
                    try {
                        const kmlListRef = db.collection('artifacts').doc(appId)
                            .collection('public').doc('data')
                            .collection('kmlList').doc(kmlLayerDocRef.id);
                        await kmlListRef.set({
                            name: fileName,
                            uploadTime: firebase.firestore.FieldValue.serverTimestamp(),
                            uploadedBy: auth.currentUser.email || auth.currentUser.uid,
                        }, { merge: true });
                        ...
                        console.warn("⚠️ 更新 kmlList 清單失敗（不影響主層上傳）:", e);
                    }
                    await updateKmlLayerSelects();
                    updatePinButtonState();
            } catch (error) {
    ...
    deleteSelectedKmlBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            try {
              const listRef = db.collection('artifacts').doc(appId)
                  .collection('public').doc('data')
                  .collection('kmlList').doc(kmlIdToDelete);
              await listRef.delete();
              console.log(`✅ 已同步刪除 kmlList 項目: ${kmlIdToDelete}`);
            } catch (e) {
              console.warn("⚠️ 刪除 kmlList 項目失敗（不影響主刪除）:", e);
            }
